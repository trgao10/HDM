...
% figure;
% scatter3(vdm.data(1,:),vdm.data(2,:),vdm.data(3,:),1,'b','filled');
% axis equal;
% hold on
% CheckIdx = 1000;
% scatter3(vdm.data(1,CheckIdx),vdm.data(2,CheckIdx),vdm.data(3,CheckIdx),10,'r','filled');
% tangent_basis = pcaBASIS(:,:,CheckIdx);
% basis = pcaBASIS(:,:,CheckIdx)+repmat(vdm.data(:,CheckIdx),1,2);
% scatter3(basis(1,:),basis(2,:),basis(3,:),10,'g','filled');
% line([vdm.data(1,CheckIdx),basis(1,1)],[vdm.data(2,CheckIdx),basis(2,1)],[vdm.data(3,CheckIdx),basis(3,1)],'color','g');
% line([vdm.data(1,CheckIdx),basis(1,2)],[vdm.data(2,CheckIdx),basis(2,2)],[vdm.data(3,CheckIdx),basis(3,2)],'color','g');
% unit_circle = vdm.data(:,index(CheckIdx,2:end));
% unit_circle = unit_circle - repmat(vdm.data(:,CheckIdx),1,size(unit_circle,2));
% unit_circle = tangent_basis*tangent_basis'*unit_circle;
% unit_circle = unit_circle./repmat(sqrt(sum(unit_circle.^2)),3,1)+repmat(vdm.data(:,CheckIdx),1,size(unit_circle,2));
...
if vdm.debug==1
    ...
    title(['(DEBUG) the first ' num2str(vdm.embedmaxdim) ' eigenvalues\newline(note the scale)']);
    axis tight;
    subplot(2,1,2); bar((rslt.lambdaC./rslt.lambdaC(1)).^(2*vdm.T));
    set(gca,'fontsize',vdm.fontsize);
    title(['(DEBUG) the diffusion behavior of the eigenvalues\newline(note the scale) T=' num2str(vdm.T)]);
    axis tight;
end
