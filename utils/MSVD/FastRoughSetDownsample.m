...
if isempty(lOptsForNrsearch.DistInfo),
    [lCount,vNet.InvIdxs] = nrsearch( cX(:,vNet.Idxs), cX, 1, 0, struct('ReturnAsArrays',1,'ReuseOpts',false,'SaveOpts',false,'FastNNSearcher',lOptsForNrsearch.FastNNSearcher) );
else
...
if cOpts.AvoidSmallClusters>0,
    ...
    if length(lSmallClusterIdxs)>0,
        ...
        [lTmpCount, vNet.InvIdxs] = nrsearch( cX(:,vNet.Idxs), cX,1, 0, struct('ReturnAsArrays',1) );
